...
# ── Stage 4: OAuth Configuration ──────────────────────────────────────────
stage "OAuth portal configuration (optional)"
say "QuoteFlow supports managed OAuth authentication for user logins."
say "You can configure an OAuth portal now, or skip if running local dev with guest bypass."
if confirm "Do you want to configure OAuth portal credentials now?"; then
  step "Enter your OAuth portal URL (e.g. https://auth.example.com):"
  ask VITE_OAUTH_PORTAL_URL "VITE_OAUTH_PORTAL_URL:"
  write_env VITE_OAUTH_PORTAL_URL "$VITE_OAUTH_PORTAL_URL"

  step "Enter backend OAuth server service URL:"
  ask OAUTH_SERVER_URL "OAUTH_SERVER_URL:"
  write_env OAUTH_SERVER_URL "$OAUTH_SERVER_URL"

  step "Enter your Application ID (App ID):"
  ask VITE_APP_ID "VITE_APP_ID:"
  write_env VITE_APP_ID "$VITE_APP_ID"

  step "Enter the Owner OpenID identifier:"
  ask OWNER_OPEN_ID "OWNER_OPEN_ID:"
  write_env OWNER_OPEN_ID "$OWNER_OPEN_ID"

  step "Enter Owner Name (e.g. Marcus):"
  ask OWNER_NAME "OWNER_NAME:"
  write_env OWNER_NAME "$OWNER_NAME"
else
  note "Skipping OAuth portal configuration; local dev guest session can be used."
fi
